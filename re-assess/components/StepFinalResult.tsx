  ...
  return (
    <div className="space-y-8 pb-20 animate-fade-in">
      <div className="bg-white p-10 rounded-2xl shadow-xl border-t-8 border-green-600">
        <InfoBox title={t('final.info.title')} variant="success">
          <span dangerouslySetInnerHTML={{__html: t('final.info.content')}}></span>
        </InfoBox>
        
        <div className="flex justify-between items-center mb-8 border-b pb-4">
          <h3 className="text-3xl font-bold text-indigo-900">{t('final.task.title')}</h3>
          <div className={`px-5 py-2 rounded-full font-bold text-sm border shadow-sm transition-colors ${allSectionsHandled ? 'bg-green-100 text-green-800 border-green-200' : 'bg-amber-100 text-amber-800 border-amber-200'}`}>
            {allSectionsHandled ? t('final.task.handled') : t('final.task.unhandled')}
          </div>
        </div>
        ...
        <div className="space-y-12">
          <div>
            <h4 className="text-2xl font-bold text-indigo-700 mb-6 border-r-4 border-indigo-600 pr-4">{t('final.student.instructions')}</h4>
                  ...
                  <div key={originalIdx} className={`p-6 rounded-2xl border-2 transition-all ${section.status === 'approved' ? 'border-green-400 bg-green-50/30' : section.status === 'removed' ? 'border-gray-200 bg-gray-50 opacity-50' : 'border-indigo-100 bg-white shadow-sm'}`}>
                    <div className="flex justify-between items-center mb-4">
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => updateSectionStatus(originalIdx, 'approved')} className={`px-4 py-1.5 rounded-lg font-bold text-xs transition-all ${section.status === 'approved' ? 'bg-green-600 text-white' : 'bg-white border text-green-600 hover:bg-green-50'}`}>{t('final.btn.approve')}</button>
                        <button onClick={() => toggleSectionEdit(originalIdx)} className={`px-4 py-1.5 rounded-lg font-bold text-xs transition-all ${section.isEditing ? 'bg-indigo-600 text-white shadow-inner' : 'bg-white border text-indigo-600 hover:bg-indigo-50'}`}>{section.isEditing ? t('final.btn.save') : t('final.btn.edit')}</button>
                        <button onClick={() => updateSectionStatus(originalIdx, 'removed')} className={`px-4 py-1.5 rounded-lg font-bold text-xs transition-all ${section.status === 'removed' ? 'bg-red-600 text-white' : 'bg-white border text-red-600 hover:bg-red-50'}`}>{t('final.btn.remove')}</button>
                      </div>
                    </div>
                    {section.isEditing ? (
          ...
          <div className="bg-gray-50 p-8 rounded-3xl border-2 border-dashed border-gray-300">
            <h4 className="text-2xl font-bold text-gray-700 mb-6 border-r-4 border-gray-500 pr-4">{t('final.lecturer.considerations')}</h4>
            <div className="space-y-6">
        ...
        <div className="mt-10 border-t pt-8">
            <div className="flex justify-between items-center mb-4">
                <h4 className="text-2xl font-bold text-indigo-900">{t('final.rubric.title')}</h4>
                {!rubric && (
                    <button 
                    ...
                    >
                        {rubricLoading ? t('final.rubric.generating') : t('final.rubric.generate')}
                    </button>
                )}
            </div>
            {rubric && (
                      ...
                      <table className="w-full text-right border-collapse min-w-[800px]">
                        <thead>
                          <tr className="bg-indigo-50 text-indigo-900">
                            <th className="p-3 border border-indigo-100 w-1/4">{t('final.rubric.col.criterion')}</th>
                            <th className="p-3 border border-indigo-100 w-1/4">{t('final.rubric.col.excellent')}</th>
                            <th className="p-3 border border-indigo-100 w-1/4">{t('final.rubric.col.good')}</th>
                            <th className="p-3 border border-indigo-100 w-1/4">{t('final.rubric.col.needsImprovement')}</th>
                          </tr>
                        </thead>
                        <tbody>
                          {rubric.map((row, idx) => (
      ...
      <div className="bg-white p-8 rounded-2xl shadow-md border border-gray-200 hide-on-print">
        ...
        <div className="flex gap-2">
          <input type="text" value={followUpQuestion} onChange={(e) => setFollowUpQuestion(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && handleSendFollowUp()} placeholder={t('final.chat.placeholder')} className="flex-1 p-4 border rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 shadow-inner" />
          <button onClick={handleSendFollowUp} disabled={loading} className="bg-indigo-600 text-white px-8 py-2 rounded-xl font-bold hover:bg-indigo-700 transition-all">{t('final.chat.send')}</button>
        </div>
      </div>
      <div className="flex gap-4 hide-on-print">
        <button onClick={onBack} className="px-10 py-4 border-2 rounded-xl font-bold hover:bg-gray-100 transition-all">{t('back')}</button>
        <button onClick={onReset} className="flex-1 bg-indigo-900 text-white py-4 rounded-xl font-bold hover:bg-black shadow-lg transition-all">{t('final.btn.finish')}</button>
      </div>
    </div>
  );
};
