  ...
  return (
    <div className="space-y-6 animate-fade-in pb-12">
      <div className="bg-white p-8 rounded-2xl shadow-xl border-t-8 border-indigo-600 relative">
        <InfoBox title={t('strategy.info.title', 'בניית תהליך ההערכה')}>
          <span dangerouslySetInnerHTML={{__html: t('strategy.info.content', 'לפניך הצעה לרצף הערכה המותאם למיומנויות שבחרת.<br/>העוזר הפדגוגי סידר את החלקים בסדר לוגי (למשל: ידע לפני יישום, או יצירה לפני הגנה), אך יש לך שליטה מלאה לשנות את השיטות והסדר.')}}></span>
        </InfoBox>
        
        <div className="flex justify-between items-center mb-6">
          <div>
            <h3 className="text-2xl font-bold text-indigo-900">{t('strategy.header', 'רצף ההערכה המוצע')}</h3>
            <p className="text-gray-600">{t('strategy.desc', 'התאם את הקבוצות לפי שיקול דעתך. העוזר מתחשב ב-{numStudents} סטודנטים.', {numStudents})}</p>
          </div>
          <button onClick={() => addNewGroup('מיומנות חדשה')} className="bg-green-600 text-white px-5 py-2.5 rounded-xl font-bold hover:bg-green-700 transition-all shadow-md">{t('strategy.btn.add.group', '+ הוסף חלק')}</button>
        </div>
          ...
          {strategies.map((strat, idx) => (
            <div key={strat.id} className="p-6 rounded-2xl bg-gray-50 border-2 border-indigo-100 shadow-sm relative overflow-hidden">
              <div className="absolute top-0 right-0 bg-indigo-100 text-indigo-800 px-4 py-1 rounded-bl-xl font-bold text-sm">
                 {t('strategy.part', 'חלק {part}', {part: idx + 1})}
              </div>
              <h4 className="text-sm font-bold text-indigo-400 mb-3 mt-4 uppercase">{t('strategy.col.skills', 'קבוצת מיומנויות')}</h4>
              ...
              {inspectedSkill && inspectedSkill.groupIdx === idx && (
                <div className="mb-6 p-5 bg-white rounded-xl border-2 border-indigo-500 shadow-lg animate-fade-in">
                  ...
                  <div className="flex flex-wrap gap-2 pt-4 border-t">
                    <span className="text-xs font-bold text-gray-400 w-full mb-1">{t('strategy.drag.hint', 'העבר לחלק:')}</span>
                    {strategies.map((target, tIdx) => target.id !== strat.id && <button key={target.id} onClick={() => { moveSkillToGroup(inspectedSkill.name, target.id); setInspectedSkill(null); }} className="bg-indigo-50 text-indigo-700 text-xs px-3 py-1.5 rounded-lg hover:bg-indigo-600 hover:text-white">{t('strategy.part', 'חלק {part}', {part: tIdx + 1})}</button>)}
                    <button onClick={() => { addNewGroup(inspectedSkill.name); setInspectedSkill(null); }} className="bg-green-50 text-green-700 text-xs px-3 py-1.5 rounded-lg hover:bg-green-600 hover:text-white">+ {t('strategy.part.new', 'חלק חדש')}</button>
                  </div>
                </div>
              )}
              <div className="grid md:grid-cols-2 gap-8 pt-4 border-t">
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-3">{t('strategy.type', 'סוג הערכה:')}</label>
                  <div className="flex gap-4">
                    <button onClick={() => updateStrategySelection(idx, 'FaceToFace')} className={`flex-1 py-4 rounded-xl font-bold border-2 transition-all ${strat.userSelectedCategory === 'FaceToFace' ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg' : 'bg-white text-gray-400 border-gray-100 hover:border-indigo-200'}`}>{t('strategy.type.facetoface', 'הערכה בסביבה מבוקרת')}</button>
                    <button onClick={() => updateStrategySelection(idx, 'Submission')} className={`flex-1 py-4 rounded-xl font-bold border-2 transition-all ${strat.userSelectedCategory === 'Submission' ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg' : 'bg-white text-gray-400 border-gray-100 hover:border-indigo-200'}`}>{t('strategy.type.submission', 'הערכה בסביבה פתוחה')}</button>
                  </div>
                </div>
                {strat.userSelectedCategory && (
                  <div className="animate-fade-in">
                    <label className="block text-sm font-bold text-gray-700 mb-3">{t('strategy.method', 'בחירת שיטה:')}</label>
                    <select 
                    ...
                    >
                      <option value="">{t('strategy.method.select', '-- בחר שיטה --')}</option>
                      {(() => {
                    ...
                    <div className="mt-3 bg-blue-50 border border-blue-100 p-3 rounded-lg">
                        <p className="text-xs font-bold text-blue-800 mb-1">{t('strategy.rationale', 'רציונל פדגוגי לשיטה ולמיקום ברצף:')}</p>
                        <p className="text-sm text-blue-900 leading-relaxed">{strat.explanation}</p>
                    </div>
                    {shouldShowDefenseSuggestion(idx, strat) && (
                      <button onClick={() => addNewGroup('כללי', true)} className="mt-3 w-full text-xs bg-amber-50 text-amber-700 border border-amber-200 p-2 rounded-lg font-bold hover:bg-amber-100 shadow-sm transition-colors">{t('strategy.btn.add.defense', '+ הצעה: הוספת שלב הגנה (Defense) בסיום התהליך')}</button>
                    )}
                  </div>
                )}
              </div>
        ...
        <div className="flex gap-4 mt-12 pt-8 border-t">
          <button onClick={onBack} className="px-10 py-4 border-2 rounded-xl font-bold hover:bg-gray-50 transition-all">{t('back', 'חזור')}</button>
          <button onClick={onNext} disabled={loading} className="flex-1 bg-green-600 text-white py-4 rounded-xl font-bold text-xl shadow-xl hover:bg-green-700 transition-all">{t('strategy.btn.generate', 'צור מטלה וסיכום למידה')}</button>
        </div>
      </div>
    </div>
  );
