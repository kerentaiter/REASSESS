...
const App: React.FC = () => {
  ...
  const startRedesign = async (initialText?: string) => {
    ...
    // If the text hasn't changed and we already have an analysis, just go to step 2
  ...
  const handleGetInitialStrategies = async () => {
    ...
    // If we already have strategies and the skills haven't changed, just go to step 3
    if (strategies.length > 0 && step === 2) {
      // Basic check if skills match (could be improved, but sufficient for simple caching)
  ...
  const handleFinalRephrase = async () => {
    ...
    // If we already have revised sections and are on step 3, just go to step 4
